...
# steamcloudfilemanager — AppImage, требует GLIBC 2.39
PACKAGES_SIMPLE=(morewaita adwyra zavesa)
PACKAGES_DEB=(github-plus happ winboat anidesk aya gitkraken)
PACKAGES_BUILD=(kitsune)
PACKAGES_APPIMAGE=(steamcloudfilemanager)
...
if [[ -n "$1" ]]; then
    ...
    apt-get install -y -qq meson python3 blueprint-compiler libgtk4-devel libadwaita-devel gettext desktop-file-utils libappstream-glib glib2-devel 2>/dev/null | tail -3

    # Тестируем все пакеты
    for pkg in "${PACKAGES_SIMPLE[@]}" "${PACKAGES_BUILD[@]}" "${PACKAGES_DEB[@]}" "${PACKAGES_APPIMAGE[@]}"; do
